...
const treatmentData = {
  ...
  doctor: {
    ...
    aclCount: "900+",
    image:
      "https://www.westend61.de/en/photo/ADSF45908/front-view-of-positive-young-african-american-female-doctor-in-eyeglasses-uniform-and-stethoscope-smiling-and-looking-at-camera-while-standing-with-clipboard-in-hands-against-white-wall-in-daylight",
  },
...
export default function ACLReconstruction() {
  ...
  // ✅ Always scroll to top when opened
  useEffect(() => {
    setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-emerald-100 text-base md:text-lg font-light">
                {treatment.shortDesc}
              </p>
            </div>
          </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div>
                ...
                <p className="text-emerald-200 italic text-sm">
                  {treatment.doctor.title} - {treatment.doctor.specialtyFocus}
                </p>
                <div className="grid grid-cols-2 lg:flex gap-3 mt-2 text-xs">
                  <p className="flex items-center gap-1 text-emerald-300">
                    <ClockIcon className="w-3 h-3" />{" "}
                    <strong>{treatment.doctor.experience}</strong> Exp
                  </p>
                  <p className="flex items-center gap-1 text-emerald-300">
                    <UsersIcon className="w-3 h-3" />{" "}
                    <strong>{treatment.doctor.patientsTreated}</strong> Patients
                  </p>
                  <p className="flex items-center gap-1 text-emerald-300">
                    <CheckCircleIcon className="w-3 h-3" />{" "}
                    <strong>{treatment.doctor.aclCount}</strong> ACL
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-3 mt-4">
            {Object.entries(treatment.stats).map(([key, value], idx) => (
              <div
                key={idx}
                className="bg-gray-900 bg-opacity-85 rounded-lg p-3 shadow-md text-center transform hover:scale-105 transition-transform duration-300"
              >
      ...
      <div className="container mx-auto px-4 py-12 max-w-5xl space-y-12">
        {/* Overview */}
        <section>
          <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-4 border-b-2 border-emerald-200 pb-1">
            Treatment Overview
          </h2>
          ...
          <div className="bg-emerald-50 border-l-4 border-emerald-500 p-4 rounded-lg shadow-sm">
            <h3 className="text-lg font-bold text-emerald-900 mb-2 flex items-center gap-2">
              <ShieldCheckIcon className="w-5 h-5 text-emerald-600" /> Who is
              this treatment for?
            </h3>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <section>
            ...
            {treatment.procedure.map((step, idx) => (
              <div
                key={idx}
                className="flex gap-3 items-start mb-4 border-b pb-2 last:border-b-0"
              >
                <div className="w-8 h-8 bg-gradient-to-br from-emerald-500 to-emerald-600 text-white rounded-full flex items-center justify-center flex-shrink-0 font-bold text-sm shadow-md mt-1">
                  {idx + 1}
                </div>
          ...
          <section>
            ...
            <ul className="space-y-2 text-gray-700 bg-white p-4 rounded-lg shadow-sm text-sm">
              {treatment.recovery.map((r, i) => (
                <li
                  key={i}
                  className="flex items-start gap-2 border-b pb-1 last:border-b-0"
                >
          ...
          <section>
            ...
            <div className="bg-red-50 border-l-4 border-red-400 p-4 rounded-lg shadow-sm text-sm">
              <p className="text-gray-700 mb-2">
                While ACL reconstruction is safe, be aware of potential risks:
              </p>
        ...
        {/* Patient Gallery */}
        <section>
          <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 mb-3 border-b-2 border-emerald-200 pb-1">
            Patient Success Stories & Results
          </h2>
          <div className="grid md:grid-cols-3 gap-4">
            {treatment.mockPatientImages.map((src, idx) => (
              <img
                ...
                src={src}
                alt={`Patient ${idx + 1}`}
                className="rounded-lg shadow-md w-full h-40 object-cover hover:shadow-lg"
              />
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {treatment.mockTestimonials.map((t, idx) => (
              <div
                key={idx}
                className="bg-white p-4 rounded-lg shadow-md border-t-4 border-emerald-500 text-sm"
              >
        ...
        <section className="bg-emerald-700 text-white p-6 rounded-xl shadow-md text-center mt-12">
          ...
          <p className="text-sm md:text-base mb-4 text-emerald-100">
            Take the first step towards a pain-free life. Book a consultation
            with Dr. Hyder Kunnummla.
          </p>
          <button
            onClick={() => alert("Navigate to Booking Page")}
            className="bg-white text-emerald-700 hover:bg-emerald-50 font-bold text-sm md:text-base py-2 px-6 rounded-full shadow-sm hover:scale-105 transition-transform"
          >
